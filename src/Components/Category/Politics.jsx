...
function Politics() {
  ...
  const fetchNewsBusiness = async (page = 1) => {
    ...
    try {
      const response = await getNewsByCategory('politics', page, pageSize, selectedDate);
          ...
          <div className="bg-[#E6E5E1] mt-[-3.7rem] p-2">
            {articles.length > 0 ? (
              articles.map((article, index) => (
                <div className="mt-4 ml-4 mr-4 relative" key={index}>
                  <Link className="flex flex-col sm:flex-row items-start" to={`/detailspage/${article.newsId}`}>
                    ...
                    <div className="w-full sm:w-[15rem] relative">
                      ...
                      <img
                        ...
                        alt={article.title}
                        className="h-[12rem] sm:h-[12rem] w-full sm:w-[15rem] rounded-md hover:scale-105 transition-transform duration-300 cursor-pointer object-cover"
                      />
                    </div>
                    <div className="mt-4 sm:mt-0 sm:ml-4 flex flex-col justify-between w-full sm:w-3/6">
                      <div className="text-xs text-gray-500 uppercase">{article.categories}</div>
                      <p className="text-sm sm:text-base font-semibold mt-2">{article.title}</p>
                      <div className="flex items-center mt-3">
                        <p className="text-xs sm:text-sm">
                          {article.pubDate.split(" ")[0] || "Mar 10, 2022"}
                        </p>
                      </div>
                      <p className="mt-3 text-sm text-gray-700">{truncateText(article.description, 160)}</p>
                    </div>
                  </Link>
