...
import { getNewsByInterNational } from "../../services/newsService"; // Assuming this function is already defined
...
function International() {
  ...
  // Fetch news for international
  const fetchNewsInternational = async (page = 1) => {
    try {
      setLoading(true);
      const newsData = await getNewsByInterNational(page, pageSize);
  ...
  useEffect(() => {
    fetchNewsInternational(currentPage);
        ...
        <>
          <div className="bg-[#E6E5E1] mt-[-3.7rem] p-2">
            {articles.map((article, index) => (
              <div className="mt-4 ml-4 mr-4" key={index}>
                <Link
                ...
                >
                  <div className="w-full sm:w-[15rem] relative">
                    ...
                    <img
                      ...
                      alt={article.title}
                      className="h-[10rem] w-full sm:w-[15rem] rounded-md hover:scale-105 transition-transform duration-300 cursor-pointer object-cover"
                    />
                  </div>
          ...
          {totalPages > 0 && (
            <div className="flex justify-center items-center mt-8 mb-8">
              <nav className="inline-flex rounded-full gap-2">
                <button
